...
public abstract class AbstractCacheStorage<CACHE, @NonNull SER_KEY> implements CacheStorage
{
	...
	/**
	 * Indicates whether the value should be serialized (because the underlying cache is going to write it, or whether it can be kept as an
	 * object
	 */
								   ...
								   @Nullable Function<String, @NonNull SER_KEY> pKeySerializer, @Nullable Function<@NonNull SER_KEY, String> pKeyDeserializer)
	{
		...
		if (!mSerializeValue)
		{
			...
			{
				throw new IllegalArgumentException("Only a SER_VALUE of MemoryStorageData.class is supported for non-serializing caches");
			}
		}
	}
			...
			while (!currentTypes.isEmpty())
			{
				...
				for (Map.Entry<Short, ByteBuffer> entry : currentTypes.entrySet())
				{
					...
					if (typeType == TYPE_PARAMETERIZED)
					{
						...
						@NonNull Type @NonNull [] actualTypeArguments = new Type[actualTypeArgumentsLen];
					...
					else if (typeType == TYPE_WILDCARD)
					{
						...
						@SuppressWarnings("null")
						@NonNull Type @Nullable [] lowerBounds = lowerBoundsLen == 0 ? null : new Type[lowerBoundsLen];
						short upperBoundsLen = valueBuffer.getShort();
						@SuppressWarnings("null")
						@NonNull Type @Nullable [] upperBounds = upperBoundsLen == 0 ? null : new Type[lowerBoundsLen];
	...
	protected <V> List<CommonKeyValuePair<CACHE, SER_KEY>> serializeEntry(KeySPI<V> pKey, CacheResult<V> pResult)
	{
		...
		if (mSerializeValue)
		{
			...
			Class<V> valueClass = !isNull ? (Class<V>) value.getClass() : (Class<V>) outputType;
		...
		return Collections.singletonList(new CommonKeyValuePair<>(mPrimaryCache, primaryKey, pKey, finalValue, overrideExpiry));
	}

	/**
	 * Provides a stream of entries from the primary. NOTE: If meta is not a separate cache, then it is expected that metadata may be present,
	 * so it's OK to return all keys
	 *
	 * @return the entries
	 */
	...
	/**
	 * Provides a stream of entries from the meta. NOTE: If meta is not a separate cache, then it is expected that regular data will be
	 * present, so it's OK to return all keys.
	 *
	 * @return the entries
	 */
	...
	protected Map.Entry<Key<?>, CacheResult<?>> deserializeEntry(SER_KEY pKey, Object pValue)
	{
		...
		if (mSerializeValue)
		{
			...
			@NonNull KeySPI<Object>[] parts = new KeySPI[keyLen];
			...
			for (int i = 0; i < keyLen; i++)
			{
				...
				if (partBaseKey.startsWith("{"))
				{
					...
					{
						parts[i] = new ResolvedAccessContextPlaceholder<>(new StaticAccessContextPlaceholder<>(partBaseKey, outputType), fullSplit[i]);
					}
					...
					else if (placeholderType == PART_TYPE_PLACEHOLDER_DEFAULTS)
					{
						...
						KeySPI<Object> r = (KeySPI<Object>) (KeySPI) new ResolvedKeyPlaceholder<>(
								new StaticKeyPlaceholderWithDefault(partBaseKey, outputType, defaultKey), fullSplit[i]);
						parts[i] = r;
					}
					else
					{
						throw new IllegalStateException("The placeholder part type(" + String.valueOf(placeholderType) + ") is not recognized");
					}
				}
				else
				{
		...
		if (id == null)
		{
			synchronized (this)
			{
				...
				if (id == null)
				{
					...
					pWriteList.add(new CommonKeyValuePair<>(mMetaCache != null ? mMetaCache : mPrimaryCache, idKey, null, idValue, null));
				}
			}
		}
		return id;
	...
	protected @Nullable String decompressString(short pId)
	{
		...
		if (str == null) throw new IllegalArgumentException("The string id (" + String.valueOf(pId) + ") is not recognized");
	...
	protected @Nullable Type decompressType(short pTypeId)
	{
		...
		if (type == null) throw new IllegalArgumentException("The type id (" + String.valueOf(pTypeId) + ") is not recognized");
